    ...
    switch (keyPress) {
        case(W):
            if(player_1.y > 0) {
                ...
                socket.emit('bluePlayerClint', {"move": 'up', "py": player_1.y})
            }
            break;
        case(S):
            if(player_1.y < gameY-player_1.height) {
                player_1.y += playerSpeed;
                socket.emit('bluePlayerClint', {"move": 'down', "py": player_1.y})
            }
            break;
        case(UP):
            if(player_2.y > 0) {
                player_2.y -= playerSpeed;
                socket.emit('redPlayerClint', {"move": 'up', "py": player_2.y})
            }
            break;
        case(DOWN):
            if(player_2.y < gameY-player_2.height) {
                player_2.y += playerSpeed;
                socket.emit('redPlayerClint', {"move": 'down', "py": player_2.y})
            }
            break;
    }
};
...
function updateScore() {
    socket.emit('scoreClint', {"blueScore":player_1_Score, "redScore": player_2_Score});
...
socket.on('bluePlayerServer', (data)=>{
    if(data.move == 'up'){
        player_1.y = data.py;
...
socket.on('redPlayerServer', (data)=>{
    if(data.move == 'up'){
        player_2.y = data.py;
    } else if(data.move == 'down'){
        player_2.y = data.py;
...
socket.on('ballServer', (data)=>{
    ballX = data.x;
    ballY = data.y;
});
socket.on('playersScore', (data)=>{
    player_1_Score = data.blueScore;
    player_2_Score = data.redScore;
});